...
static void glnvg__bindTexture(GLNVGcontext *gl, GLuint tex, int msaa)
{
#if NANOVG_GL_USE_STATE_FILTER
...
static void nvgGLLog(GLNVGcontext *gl, NVGLogLevel level, const char *msg)
{
	nvgUserImplLog(gl->userImplementation, level, msg);
}

static void nvgGLLogFmt(GLNVGcontext *gl, NVGLogLevel level, const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
#ifndef NVGGL_LOG_LINE
#define NVGGL_LOG_LINE(gl, level, msg) nvgGLLogFmt(gl, level, "%s from %s in %s at %d", msg, __func__, __FILE__, __LINE__)
#endif
...
//
// https://stackoverflow.com/questions/29813500/cant-release-textures-created-by-a-shared-context
//
// If you really want to delete the texture, you'll have to make
// a glDeleteTexture() call. It does not matter if you make this call in
...
static void glnvg__dumpShaderError(GLNVGcontext *gl, GLuint shader, const char *name, const char *type)
{
	...
	if (infoLogLength < 1 || infoLogLength == INT_MAX)
	{
		NVGGL_LOG_LINE(gl, NVG_LOG_LEVEL_ERROR, "Invalid length for shader info log");
		return;
	}

	char *buf = (char *)malloc(infoLogLength);

	GLint len;
	glGetShaderInfoLog(shader, infoLogLength, &len, buf);
	GLint npLen = len + 1;

	if (npLen != infoLogLength)
	{
		NVGGL_LOG_LINE(gl, NVG_LOG_LEVEL_ERROR, "Invalid length for shader info log");
	}
	else
	{
		nvgGLLogFmt(gl, NVG_LOG_LEVEL_ERROR, "Shader %s/%s error:\n%s\n", name, type, buf);
	}
...
static void glnvg__dumpProgramError(GLNVGcontext *gl, GLuint prog, const char *name)
{
	...
	if (infoLogLength < 1 || infoLogLength == INT_MAX)
	{
		NVGGL_LOG_LINE(gl, NVG_LOG_LEVEL_ERROR, "Invalid length for program info log");
		return;
	}
	...
	GLint len;
	glGetProgramInfoLog(prog, infoLogLength, &len, buf);
	GLint npLen = len + 1;
	...
	{
		nvgGLLogFmt(gl, NVG_LOG_LEVEL_ERROR, "Program %s error:\n%s\n", name, buf);
	}
...
static void glnvg__checkError(GLNVGcontext *gl, const char *str)
{
	...
	if (err != GL_NO_ERROR)
	{
		nvgGLLogFmt(gl, NVG_LOG_LEVEL_ERROR, "Error %08x after %s\n", err, str);
	}
}

static int
glnvg__createShader(GLNVGcontext *gl, GLNVGshader *shader, const char *name, const char *header, const char *opts, const char *vshader,
					const char *fshader)
{
	GLint status;
...
//
// does not delete the texture, but frees the nvgImage handle
//
...
#endif
{
	...
	NVGUserImplementation *userImplementation = (NVGUserImplementation *)malloc(sizeof(NVGUserImplementation));
	memset(userImplementation, 0, sizeof(NVGUserImplementation));
	...
	{
	case 0:
		params.contextType = NVG_FULL_CONTEXT;
	...
	if (params.contextType == NVG_FULL_CONTEXT)
	{
		...
		gl->userImplementation = userImplementation;
	...
	ctx = nvgCreateInternal(&params, userImplementation);
	if (ctx == NULL)
		goto error;
